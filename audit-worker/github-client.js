...
function GithubClient(config) {
  ...
  async function fetchGithubDiff(domain, latestAuditTime, lastAuditedAt, gitHubURL) {
    if (!gitHubURL) {
      log('info', `No github repo defined for site ${domain}. Skipping github diff calculation`);
      return '';
    }
    ...
    try {
      ...
      log('info', `Fetching diffs for domain ${domain} with repo ${repoPath} between ${since.toISOString()} and ${until.toISOString()}`);
      ...
      for (const sha of commitSHAs) {
        log('info', `Fetching diff for commit ${sha} for site ${domain}`);
        ...
        if (!diffResponse.data.includes("Binary files differ") && (totalSize + diffResponse.data.length) < MAX_DIFF_SIZE) {
          ...
          log('info', `Added commit ${sha} (${totalSize} of ${MAX_DIFF_SIZE}) to diff for site ${domain}.`);
        } else {
          log('warn', `Skipping commit ${sha} because it is binary or too large (${totalSize} of ${MAX_DIFF_SIZE}) for site ${domain}.`);
          break;
        }
      }
      ...
      log('error', `Error fetching GitHub diff data for site ${domain}:`, error.response ? error.response.data : error);
      return '';
    }
  }
